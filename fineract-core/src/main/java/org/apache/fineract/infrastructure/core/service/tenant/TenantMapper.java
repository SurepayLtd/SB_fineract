...
public final class TenantMapper implements RowMapper<FineractPlatformTenant> {
    ...
    private static final String TENANT_SERVER_CONNECTION_BUILDER = " t.id, ts.id as connectionId , "
            + " t.timezone_id as timezoneId , t.name,t.identifier, t.two_factor_enabled as twoFactorEnabled, t.agent_name as agentName, "
            + " t.agent_code as agentCode, ts.schema_name as schemaName, ts.schema_server as schemaServer,"
    ...
    public FineractPlatformTenant mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
        ...
        final String agentCode = rs.getString("agentCode");
        return new FineractPlatformTenant(id, tenantIdentifier, name, timezoneId, connection, twoFactorEnabled, agentName, agentCode);
    }
